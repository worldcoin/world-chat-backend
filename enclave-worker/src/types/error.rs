...
impl From<enclave_types::EnclaveError> for AppError {
    fn from(err: enclave_types::EnclaveError) -> Self {
        ...
        let (code, message, allow_retry) = match err {
            EnclaveError::NotInitialized => {
                ("enclave_not_initialized", "Enclave not initialized", true)
            }
            EnclaveError::UnexpectedResponse => (
                "unexpected_response",
        ...
        tracing::error!("Enclave error: {err}");
        Self::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            code,
            message,

...
pub async fn handler(
    ...
    let braze_payload = json!({
        "user_aliases": user_aliases,
        "messages": {
            "apple_push": request.apple_push,
            "android_push": request.android_push
        }
    });

    send_braze_notification(client, braze_api_key, braze_api_endpoint, braze_payload).await?;
...
async fn send_braze_notification(
    ...
    braze_payload: serde_json::Value,
) -> Result<(), EnclaveError> {
    let body = Body::from(braze_payload.to_string());

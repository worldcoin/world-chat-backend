...
pub struct SubscriptionRequestQueue {
    sqs_client: Arc<SqsClient>,
    config: QueueConfig,
}

impl SubscriptionRequestQueue {
    /// Creates a new subscription request queue
    ///
    /// # Arguments
    ///
    ...
    /// Sends a subscription request message to the queue
    ///
    /// # Arguments
    ///
    /// * `message` - The subscription request to send
    ///
    /// # Returns
    ///
    ...
    pub async fn send_message(&self, message: &SubscriptionRequest) -> QueueResult<String> {
    ...
    /// # Returns
    ///
    /// A vector of subscription request messages
    ///
    /// # Errors
    ///
    /// Returns `QueueError` if the poll operation fails
    pub async fn poll_messages(&self) -> QueueResult<Vec<QueueMessage<SubscriptionRequest>>> {
        ...
        let messages = result
            ...
            .filter_map(|msg| {
                ...
                match serde_json::from_str::<SubscriptionRequest>(body) {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "Failed to deserialize subscription request message: {}",
                            e
                        );
                        None
                    }
